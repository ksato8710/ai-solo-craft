...
└──────┬──────────────┬──────────────┬─────────────────┘
       ...
       ▼              ▼              ▼
┌──────────────┐ ┌──────────┐ ┌──────────────┐ ┌───────────┐
│   Supabase   │ │  Resend  │ │ Vercel Cron  │ │   Slack   │
│ (PostgreSQL) │ │ (Email)  │ │  (23:15 UTC) │ │  (通知)   │
└──────────────┘ └──────────┘ └──────────────┘ └───────────┘
```
...
| 運用通知 | **Slack API** | 登録/確認イベントをリアルタイム通知、既存 Bot Token 流用 |
...
| 副作用 | DB upsert + Resend 確認メール送信 + Slack 通知 |
...
5. 確認メール送信（Resend）
6. Slack 通知（`📬 ニュースレター新規登録: {email}`、fire-and-forget）
...
| 処理 | `status` → `active`、`verified_at` 記録、Slack 通知 |
...
| `SLACK_BOT_TOKEN` | Slack Bot Token（`xoxb-...`）| `.env.local` + Vercel |
| `SLACK_NOTIFICATION_CHANNEL` | 通知先 Slack チャネル ID | `.env.local` + Vercel |

---

## 9. Slack 通知

登録・確認イベントをリアルタイムで Slack チャネルに通知する。

### 9.1 通知タイミング

| イベント | メッセージ | トリガー |
|---------|-----------|---------|
| 新規登録 | `📬 ニュースレター新規登録: {email}` | `/api/newsletter/subscribe`（既存 active を除く） |
| 確認完了 | `✅ ニュースレター登録確認完了（メール認証済み）` | `/api/newsletter/confirm`（成功時） |

### 9.2 実装

- **関数:** `notifySlack(text)` in `src/lib/newsletter.ts`
- **API:** Slack `chat.postMessage`（`fetch` で直接呼び出し、追加依存なし）
- **方式:** fire-and-forget（通知失敗は登録処理に影響しない）
- **チャネル:** `C0ACUDZCJJC`（環境変数 `SLACK_NOTIFICATION_CHANNEL` で指定）
...
1. Resend API でドメイン追加: `POST https://api.resend.com/domains` （Full Access キーが必要）
2. 返却される DNS レコード（DKIM: TXT, SPF: MX + TXT）を Route 53 ホストゾーンに追加
3. `POST https://api.resend.com/domains/{id}/verify` で検証トリガー
4. 認証完了後、`from` アドレスとして `newsletter@ai.essential-navigator.com` が使用可能に

**DNS 管理:**
- ホストゾーン: `Z063638914X04N4ZLWPVT`（`essential-navigator.com`）
- NS: `ns-1447.awsdns-52.org`, `ns-22.awsdns-02.com`, `ns-831.awsdns-39.net`, `ns-1745.awsdns-26.co.uk`
- Resend レコード: `resend._domainkey.ai.essential-navigator.com` (TXT/DKIM), `send.ai.essential-navigator.com` (MX + TXT/SPF)
- Vercel: `ai.essential-navigator.com` → CNAME → `3a7426ab097d534b.vercel-dns-017.com`
